...
public:
    explicit TestTcpServer(Node* parent = nullptr) : TcpServer(parent) {
        newConnection.connect([this]() { signalNewConnectionCount++; });
    }
...
public:
    explicit TestTcpSocket(Node* parent = nullptr) : TcpSocket(parent) {
        socketClosed.connect([this](AbstractSocket*) { signalSocketClosedCount++; });
...
protected:
    void SetUp() override { app = new Application(); }
...
TEST_F(NetworkFixture, AppInitializer) { EXPECT_EQ(Application::instance(), app); }
...
TEST_F(NetworkFixture, TcpServerListen) {
    ...
    EXPECT_TRUE(server.getPort().has_value());
...
TEST_F(NetworkFixture, TwoTcpServerListenSamePort) {
    TcpServer server;
    ASSERT_TRUE(server.listen(1234));
    ASSERT_TRUE(server.isListening());
    TcpServer server2;
    ASSERT_FALSE(server2.listen(1234));
    ASSERT_FALSE(server2.isListening());
    ASSERT_FALSE(server2.getPort().has_value());

    // Try to connect and disconnect the same TcpServer
    TcpServer server3;
    for (int i = 0; i < 60; ++i) {
        EXPECT_TRUE(server3.listen(1231));
        server3.close();
        EXPECT_FALSE(server3.isListening());
        EXPECT_FALSE(server3.getFileDescriptor().has_value());
    }

    // In the same way, creating and destroying the
    // server should be available to listen again
    // on the same port
    for (int i = 0; i < 60; ++i) {
        TcpServer test;
        EXPECT_TRUE(test.listen(1231));
    }
}
...
TEST_F(NetworkFixture, TcpServerNextPendingConnection) {
    ...
    ASSERT_TRUE(ioContext);

    TestTcpSocket socketClient;
    socketClient.connect("127.0.0.1", 1234);
    socketClient.waitForConnected(9000);
    ASSERT_TRUE(socketClient.getIsConnected());
    using namespace std::chrono;
    auto start = high_resolution_clock::now();
    auto end_time = start + seconds(2);
    ...
    ASSERT_TRUE(socketServer != nullptr);
    ASSERT_EQ(server.signalNewConnectionCount, 1);
    ASSERT_TRUE(socketClient.getIsConnected());
    ASSERT_TRUE(socketServer->getIsConnected());
    ASSERT_EQ(socketServer->parent(), &server);
    ASSERT_EQ(server.childrenCount(), 1);
    ASSERT_TRUE(socketServer);
